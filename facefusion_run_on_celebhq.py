...
from facefusion.vision import get_video_frame, read_image, read_static_image, read_static_images, pack_resolution, detect_video_resolution, detect_video_fps, create_video_resolutions
...
def cli() -> None:
	...
	program.add_argument('-s', '--source', help = wording.get('source_help'), dest = 'source_paths', default = 'source_images.pickle')  # updated
	...
	group_misc.add_argument('--headless', help = wording.get('headless_help'), action = 'store_true', default = True)
...
def run(program : ArgumentParser) -> None:
	...
	if facefusion.globals.source_paths == 'source_images.pickle':
		global source_image_paths
		with open (facefusion.globals.source_paths, 'rb') as fp:
			source_image_paths = pickle.load(fp)
		# source_image_paths = source_image_paths[1:5]  # TODO temporary, remove
		# printlist(source_image_paths)
		process_image()  # swap and enhance
	else:
		process_image_single_source()
...
# updated by Sanjay Saha
def conditional_append_reference_faces2(s_path, t_path, frame_processor_modules) -> None:
	clear_reference_faces()
	source_frames = read_static_images(s_path)
	source_face = get_average_face(source_frames)
	reference_frame = read_image(t_path)
	reference_face = get_one_face(reference_frame, facefusion.globals.reference_face_position)
	append_reference_face('origin', reference_face)
	if source_face and reference_face:
...
def get_output_path(target_path, source_path):
	target_filename = os.path.basename(target_path)
	target_id = os.path.basename(os.path.dirname(target_path))
	source_id = os.path.basename(os.path.dirname(source_path))
	# print(target_id, source_id)
	output_dir = os.path.join(facefusion.globals.output_path, target_id+'_'+source_id)
	# print(facefusion.globals.output_path, '\n\n')
	# print(output_dir,'\n\n')
	Path(output_dir).mkdir(parents=True, exist_ok=True)
	final_output_path = os.path.join(output_dir, target_filename)
...
def process_image() -> None:
	...
	logger.info(wording.get('processing'), face_swapper_module.NAME)

	for t_img_name in tqdm(os.listdir(facefusion.globals.target_path), desc='Target'):
		t_path = os.path.join(facefusion.globals.target_path, t_img_name)

		# if analyse_image(facefusion.globals.target_path):
		# return
		for s_path in source_image_paths:
			start_time = time.time()
			if os.path.dirname(t_path) == os.path.dirname(s_path):  # same ID
				continue
			o_path = get_output_path(t_path, s_path)
			if is_image(o_path):
				continue
			
			shutil.copy2(t_path, o_path)
			
			conditional_append_reference_faces2([s_path], t_path, [face_swapper_module, face_enhancer_module])
			face_swapper_module.process_image([s_path], o_path, o_path)
			read_static_image.cache_clear()
			face_enhancer_module.process_image([s_path], o_path, o_path)
			read_static_image.cache_clear()
			# frame_processor_module.post_process()
			
			# compress image
			# logger.info(wording.get('compressing_image'), __name__.upper())
			# if not compress_image(o_path):
	...
	face_swapper_module.post_process()
	face_enhancer_module.post_process()


def process_image_single_source() -> None:
	face_swapper_module = load_frame_processor_module('face_swapper')
	face_enhancer_module = load_frame_processor_module('face_enhancer')
	logger.info(wording.get('processing'), face_swapper_module.NAME)

	for t_img_name in os.listdir(facefusion.globals.target_path):
		start_time = time.time()
		t_path = os.path.join(facefusion.globals.target_path, t_img_name)
		o_path = get_output_path(t_path, facefusion.globals.source_paths)
		if is_image(o_path): continue
		
		shutil.copy2(t_path, o_path)
		
		conditional_append_reference_faces2([facefusion.globals.source_paths], t_path, [face_swapper_module, face_enhancer_module])
		face_swapper_module.process_image([facefusion.globals.source_paths], o_path, o_path)
		read_static_image.cache_clear()
		face_enhancer_module.process_image([facefusion.globals.source_paths], o_path, o_path)
		read_static_image.cache_clear()
